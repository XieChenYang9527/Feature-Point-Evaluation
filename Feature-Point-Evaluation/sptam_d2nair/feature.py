...
class ImageFeature(object):
    ...
    def extract_d2net(self):
            # 提取关键点，得分和描述符
            keypoints, scores, _ = extract_keypoints(self.image)

            # 将得分和关键点结合起来
            scored_keypoints = list(zip(scores, keypoints))

            # 根据得分对结合的列表进行排序，得分最高的排在最前面
            scored_keypoints.sort(key=lambda x: x[0], reverse=True)

            # 取得分最高的前1000个点
            top_scored_keypoints = scored_keypoints[:1000]

            keypoints_opencv = []
            # 对于每一个关键点，创建一个OpenCV的关键点对象
            for _, keypoint in top_scored_keypoints:
                keypoint = cv2.KeyPoint(keypoint[0], keypoint[1], _)
                keypoints_opencv.append(keypoint)

            self.keypoints = keypoints_opencv
            return self.keypoints


    def extract(self):
        self.keypoints = self.extract_d2net()
