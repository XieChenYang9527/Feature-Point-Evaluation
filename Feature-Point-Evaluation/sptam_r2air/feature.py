...
from r2d2ce import process_image
...
class ImageFeature(object):
    ...
    def extract_r2d2(self):
        # 提取关键点和描述符
        xys, desc = process_image(self.image)
        # 只获取前1000个点
        xys = xys[:1000]
        ...
        for i in range(len(xys)):
            keypoint = cv2.KeyPoint(xys[i][0], xys[i][1], np.linalg.norm(desc[i]))
            keypoints.append(keypoint)
        self.keypoints = keypoints
        return self.keypoints

    def extract(self):
        self.keypoints = self.extract_r2d2()
