...
class EuRoCDataset(object):  # Stereo + IMU
    ...
    def list_imgs(self, dir):
        ...
        timestamps = [float(_[:-4]) * 1e-18 for _ in xs]
...
class KITTIOdometry(object):   # without lidar
    ...
    def __init__(self, path):
       ...
       # cam04_12 = Cam(369.1177, 611.5383, 314.1989, 158.3657, 640, 320, 0.53715)
...
class TartanAirOdometry(object):

    def __init__(self, path):
        # Named tuple for TartanAir Camera parameters.
        Cam = namedtuple('cam', 'fx fy cx cy width height baseline')
        tartanair_cam = Cam(320.0, 320.0, 320.0, 240.0, 640, 480, 0.5371)

        path = os.path.expanduser(path)
        timestamps = np.loadtxt(os.path.join(path, 'time.txt'))

        # Using just file lists without wrapping them into an ImageReader
        self.left = ImageReader(self.listdir(os.path.join(path, 'image_left')),
                                timestamps)
        self.right = ImageReader(self.listdir(os.path.join(path, 'image_right')),
                                 timestamps)

        # Assuming the timestamps are ordered and the length matches with the image files
        assert len(self.left) == len(self.right) == len(timestamps)
        self.timestamps = timestamps
        self.cam = tartanair_cam

    def generate_timestamps(self, dir_path):
        # Assuming timestamps can be derived from the image filenames.
        filenames = [f for f in os.listdir(dir_path) if f.endswith('.png')]
        timestamps = [float(f[:-4]) for f in filenames]  # remove ".png" and convert to float
        return sorted(timestamps)

    def listdir(self, dir):
        files = [f for f in os.listdir(dir) if f.endswith('.png')]
        return [os.path.join(dir, f) for f in sorted(files)]
