...
class ParamsEuroc(Params):
    ...
    def __init__(self, config='fast-brief'):
        super().__init__()
        ...
        if config == 'GFTT-BRIEF':
            ...
            self.feature_detector = cv2.ORB_create(
                nfeatures=1500, scaleFactor=1.1, nlevels=15, edgeThreshold=31)

            self.descriptor_extractor = self.feature_detector
        elif config == 'sifi-brief':
            self.feature_detector = cv2.SIFT_create(
                nfeatures=1000, nOctaveLayers=6, contrastThreshold=0.04, edgeThreshold=31, sigma=1.6
            )
            self.descriptor_extractor = cv2.xfeatures2d.BriefDescriptorExtractor_create(
                bytes=32, use_orientation=False)
            ...
            self.feature_detector = cv2.FastFeatureDetector_create(
                threshold=20, nonmaxSuppression=True, type=cv2.FAST_FEATURE_DETECTOR_TYPE_9_16
            )
            # 创建BRIEF描述符提取器
            self.descriptor_extractor = cv2.xfeatures2d.BriefDescriptorExtractor_create(
                bytes=32, use_orientation=False
